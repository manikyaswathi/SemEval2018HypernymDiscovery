...
def train(config, model, data, callback=lambda: None):
    ...
    with tf.Session(config=config) as sess:
        ...
        print('Cluster %d done in %s.' % (data.cluster + 1, str(t_delta)))
        callback(sess)
...
def main(_):
    ...
    if os.path.isfile('%s.test.npz' % FLAGS.model):
        print('Removing a stale file: "%s".' % ('%s.test.npz' % FLAGS.model))
    ...
    for cluster in range(kmeans.n_clusters):
        ...
        print('Writing the output model to "%s".' % saver_path)
